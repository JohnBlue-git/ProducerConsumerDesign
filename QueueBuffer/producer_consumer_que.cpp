...
void consumer(QueBuffer& rb, int consumer_id, bench::StartGate& gate) {
    gate.arrive_and_wait();
    int item = 0;
    while (rb.pop(item)) {
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < CONSUMER_COUNT; ++i) {
        consumers.emplace_back(consumer, std::ref(que), i + 1, std::ref(gate));
    }
    ...
    bench::print_json_result("queuebuffer", runtime_s, cpu_s, operations_processed.load());
    return 0;
}
